...
public class OverdraftAccountSpecs {
	...
	public void setUp() throws Exception {
		account=new OverdraftAccount(1,name,correctPassword,balance,"Overdraft");
	...
	public void creditInterstCreditsStandardInterest() {
		...
		assertEquals(expected, account.getBalance(correctPassword),0.01);
	}
	...
	public void odLimitDoesntChangeOnWithdrawal() {
		...
		assertEquals(0, account.getBalance(correctPassword),0.01); //proves withdraw was successful.
	...
	public void odLimitDoesntChangesOnDepositWhenBalanceDoesntCrossHistoricMaxBalance() {
		...
		double historicMaxBalance= account.getBalance(correctPassword);
		...
		assertEquals(balance, account.getBalance(correctPassword),0.01);
		
		//ACT
		double smallSum=10000;
		account.deposit(smallSum);
		assertEquals(balance+smallSum, account.getBalance(correctPassword),0.01);
	...
	public void odLimitChangesOnCreditInterestWhenBalanceCrossHistoricMaxBalance() {
		account.creditInterest(interestRate);
		double balance=account.getBalance(correctPassword);
	...
	public void withdrawCanWithdrawUptoBalancePlusOdLimit() {
		...
		assertTrue(account.getBalance(correctPassword)<0);
	...
	public void withdrawMoreThanBalanceGet1PercentFee() {
		...
		assertEquals(finalBalance, account.getBalance(correctPassword),0.01);
	}
	...
	public void withdrawFailsForWrongPassword() {
		...
		assertEquals(balance, account.getBalance(correctPassword),0);
